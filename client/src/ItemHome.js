...
function App() {
    ...
    useEffect(() => {
        ...
        try{
            const regCart = await axios.get("http://1.221.120.194:8282/freshCart/list");
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('http://1.221.120.194:8282/productItem/list');
        ...
        <Router>
            <div>
                ...
                <Routes>
                    <Route
                    path='/add'
                    element={<AddFreshItem onAddFreshItem={handleAddFreshItem} />}
                    />
                    <Route path="/list" element={<h1>물품 리스트</h1>}/>
